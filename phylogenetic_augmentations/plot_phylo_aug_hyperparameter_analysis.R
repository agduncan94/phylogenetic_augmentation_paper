...
drosophila_corr_summary_dev_df <- data_summary(drosophila_corr_df, varname="pcc_test_Dev", 
                                               groupnames=c("species", "type"))
plot_dev <- ggplot(drosophila_corr_summary_dev_df, aes(x=species, y=pcc_test_Dev, colour=type, fill=type)) +
...
drosophila_corr_summary_hk_df <- data_summary(drosophila_corr_df, varname="pcc_test_Hk", 
                                              groupnames=c("species", "type"))
plot_hk <- ggplot(drosophila_corr_summary_hk_df, aes(x=species, y=pcc_test_Hk, colour=type, fill=type)) +
  geom_point(data=drosophila_corr_df, size=2, position = position_dodge(width=0.9)) +
...
plot <- plot_grid(plot_dev, plot_hk, ncol=1)
...
# Add shared legend
legend <- grobs[[which(sapply(grobs, function(x) x$name) == "guide-box")]]
plot_b <- plot_grid(plot, legend, nrow = 2, rel_heights = c(1, .1))

# Final plot
figure <- plot_grid(plot_a, plot_b, ncol=1, labels=c('A', 'B'), rel_heights = c(1, 0.7,  .1))

# Save a high quality and low quality image
ggsave("../figures/phylo_aug_figure_4.tiff", figure, units="in", width=4, height=4, device='tiff', dpi=350)
ggsave("../figures/phylo_aug_figure_4.jpg", figure, units="in", width=4, height=4)
