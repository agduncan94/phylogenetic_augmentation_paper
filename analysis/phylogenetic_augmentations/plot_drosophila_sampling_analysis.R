...
drosophila_pcc <- read_tsv("../output_drosophila/model_correlation.tsv")
drosophila_pcc$type <- factor(drosophila_pcc$type)
drosophila_pcc$percent <- factor(drosophila_pcc$percent)
...
drosophila_corr_summary_dev_df <- data_summary(drosophila_pcc, varname="pcc_test_Dev", 
                                               groupnames=c("type", "percent"))

plot_a <- ggplot(drosophila_corr_summary_dev_df, aes(x=percent, y=pcc_test_Dev, colour=type, fill=type)) +
...
drosophila_corr_summary_hk_df <- data_summary(drosophila_pcc, varname="pcc_test_Hk", 
                                               groupnames=c("type", "percent"))
plot_b <- ggplot(drosophila_corr_summary_hk_df, aes(x=percent, y=pcc_test_Hk, colour=type, fill=type)) +
  geom_point(data=drosophila_pcc, size=2, position = position_dodge(width=0.9)) +
...
final_plot
ggsave(file="./output/sampled_training_data_test_performance.pdf", plot=final_plot, width=6.75, height=9)
