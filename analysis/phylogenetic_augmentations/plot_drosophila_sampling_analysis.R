...
drosophila_pcc$type <- fct_recode(drosophila_pcc$type, `No augmentations` = "none", `Augmentations` = "homologs_finetune")
...
plot_b <- ggplot(drosophila_corr_summary_hk_df, aes(x=fraction, y=pcc_test_Hk, colour=type, fill=type)) +
  ...
  #xlab("Fraction of original training data") +
  xlab("") +
  #ylab("Hk test set performance (PCC)") +
  ylab("") +
...
final_plot <- ggdraw(add_sub(final_plot, "Fraction of original training data", vpadding=grid::unit(0,"lines"),y=9, x=0.5, vjust=4.5))
final_plot
#ggsave(file="./output/sampled_training_data_test_performance.svg", plot=final_plot, width=10, height=8)