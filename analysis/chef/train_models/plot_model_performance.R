...
# Summarize the data
chef_corr_summary_h3k27ac_df <- data_summary(chef_corr_df, varname="pcc_test_h3k27ac", 
                                               groupnames=c("model", "type"))
plot_a <- ggplot(chef_corr_summary_h3k27ac_df, aes(x=model, y=pcc_test_h3k27ac, colour=type, fill=type)) +
  geom_point(data=chef_corr_df, size=2, position = position_dodge(width=0.9)) +
  geom_errorbar(aes(ymin = pcc_test_h3k27ac-sd, ymax = pcc_test_h3k27ac+sd), width=.4, position=position_dodge(.9)) +
  theme_bw() +
